 ...
 * @param {Object} keys - The current keyboard state (key -> boolean)
 */
export function update(gameState, keys) {
    ...
    if (newAxisRequested && player.movementAxis && newAxisRequested !== player.movementAxis && isOnGround) {
    ...
    if (player.velRow === 0 && player.velCol === 0) {
        ...
        if (Math.abs(player.row - targetRow) < SNAP_THRESHOLD && 
